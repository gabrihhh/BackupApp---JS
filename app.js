...
baixar.addEventListener('click',(e)=>{
     ...
     listaSalvar.forEach((e) => {
        if(localStorage.getItem(e)!=null){
            let teste = JSON.parse(localStorage.getItem(e));
            texto.push(teste.log+" - "+teste.nome+" - "+teste.horario);
        }
    });
...
carregar.addEventListener('click',(e)=>{
    listaSalvar.forEach((e)=>{
        if(localStorage.getItem(e)!=null){
            let teste = JSON.parse(localStorage.getItem(e));
            document.getElementById(e).innerText += " - "+teste.nome+" - "+teste.horario;
            document.getElementById(teste.log).classList.add("remover");
        }
    })
})